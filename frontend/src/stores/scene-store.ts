...
const initialAgents: Agent[] = [
  { id: "ana", name: "Ana", role: "Engenharia", description: "Implementa e revisa serviços.", color: 0x5ca6d8, status: "working", direction: "south", position: { x: 8, y: 6 }, basePosition: { x: 8, y: 6 }, skills: ["fastapi"], task: "Validando adapter Codex" },
  { id: "bruno", name: "Bruno", role: "Qualidade", description: "Cria testes e avalia mudanças.", color: 0xd18b64, status: "seated", direction: "south", position: { x: 14, y: 8 }, basePosition: { x: 14, y: 8 }, skills: ["testing"] },
];
...
export const useSceneStore = create<SceneStore>((set) => ({
  ...
  addAgent: (name, role) => set((state) => {
    ...
    return { agents: [...state.agents, { id, name, role, description: "Novo agente do workspace.", color: [0x85ba82, 0xa786d4, 0xe0ae59][index % 3], status: "idle", direction: "south", position: { x: 5 + index * 2, y: 10 }, basePosition: { x: 5 + index * 2, y: 10 }, skills: [] }], selectedId: id };
  }),
  removeSelected: () => set((state) => ({ agents: state.agents.filter((agent) => agent.id !== state.selectedId), selectedId: undefined })),
  assignSkill: (agentId, skillId) => set((state) => ({ agents: state.agents.map((agent) => agent.id === agentId && !agent.skills.includes(skillId) ? { ...agent, skills: [...agent.skills, skillId] } : agent) })),
