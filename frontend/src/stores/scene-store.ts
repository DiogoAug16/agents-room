...
const initialAgents: Agent[] = [
  { id: "ana", name: "Ana", role: "Engenharia", description: "Implementa e revisa serviços.", color: 0x5ca6d8, status: "working", direction: "north", position: { x: 9, y: 12 }, basePosition: { x: 9, y: 12 }, skills: ["fastapi"], skillStates: [{ id: "fastapi", enabled: true }], pluginStates: [], task: "Validando adapter Codex" },
  { id: "bruno", name: "Bruno", role: "Qualidade", description: "Cria testes e avalia mudanças.", color: 0xd18b64, status: "seated", direction: "north", position: { x: 10, y: 4 }, basePosition: { x: 10, y: 4 }, skills: ["testing"], skillStates: [{ id: "testing", enabled: true }], pluginStates: [] },
];
...
export const useSceneStore = create<SceneStore>((set) => ({
  ...
  addAgent: (name, role) => set((state) => {
    ...
    const positions = [{ x: 9, y: 12 }, { x: 10, y: 4 }, { x: 15, y: 10 }, { x: 19, y: 10 }, { x: 13, y: 16 }, { x: 16, y: 14 }, { x: 11, y: 18 }, { x: 16, y: 12 }]; const position = positions[index];
