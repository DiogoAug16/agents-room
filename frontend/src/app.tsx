...
export function App() {
  ...
  const requestInteraction = () => {
    ...
    if (interactionKind === "delegation" && !activeTask) { setEvents((items) => ["Crie uma tarefa ativa antes de delegar.", ...items].slice(0, 10)); return; }
    ...
    void request.catch(() => setEvents((items) => [interactionKind === "delegation" ? "Delegação bloqueada por limite, ciclo ou agente ocupado." : "Interação indisponível: um agente já está ocupado.", ...items].slice(0, 10)));
  };
