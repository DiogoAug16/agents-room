...
export function App() {
  ...
  useEffect(() => {
    ...
    socket.onmessage = ({ data }) => {
      ...
      if (event.type.startsWith("agent.") || event.type.startsWith("skill.") || event.type.startsWith("plugin.")) void invalidateAgents();
  ...
  return <DndContext onDragEnd={onDragEnd}><main className="app-shell">
    <header className="topbar"><div className="brand"><span className="brand-mark">AR</span><div><strong>Agents Room</strong><small>workspace local</small></div></div><div className="workspace-meta"><span>Projeto: agents-room</span><span>Branch: sem Git</span><span className={workspaceQuery.data ? "codex-online" : "codex-offline"}>● {workspaceQuery.data ? "Codex disponível" : "Backend desconectado"}</span><span>{agents.length}/8 agentes</span></div><div className="topbar-actions"><button className={editMode ? "button active" : "button"} onClick={toggleEdit}>{editMode ? "Concluir edição" : "Editar sala"}</button><AddAgentDialog onCreate={createAgent} /></div></header>
