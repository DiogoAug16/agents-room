...
import { Activity, Bot, Box, Check, CircleHelp, ClipboardList, Command, Edit3, Eye, Grid3X3, Home, Layers3, Moon, PanelLeftClose, Search, Settings, Sun, X, ZoomIn, ZoomOut } from "lucide-react";
...
import { FURNITURE_ASSETS, assignedAgentIdForFurnitureGroup, furnitureAsset, furnitureOrientations, furnitureSeats, type FurnitureCategory } from "./scene/furniture/catalog";
import { ignoresWorkspaceShortcut, panelForShortcut, toggledPanel, type PrimaryPanel } from "./ui/workspace-ui-state";

type Theme = "dark" | "light" | "system";

const PANEL_LABEL: Record<Exclude<PrimaryPanel, null>, string> = {
  overview: "Visão geral",
  agents: "Agentes",
  tasks: "Tarefas",
  capabilities: "Skills e plugins",
  furniture: "Galeria de móveis",
  activity: "Atividade",
...
function GroupNameEditor({ group, memberCount, members, selectedMemberCount, assignedAgent, assignedSeatName, onRename, onSelectAll, onFocus, onSelectAssignedAgent, onFocusAssignedSeat }: { group: { id: string; name: string; instanceIds: string[]; groupType: "workstation" | "lounge" | "meeting" | "break_area" | "partition" | "custom" }; memberCount: number; members: Array<{ id: string; name: string; category: string }>; selectedMemberCount: number; assignedAgent?: { id: string; name: string; role: string }; assignedSeatName?: string; onRename: (name: string) => void; onSelectAll: () => void; onFocus: () => void; onSelectAssignedAgent: () => void; onFocusAssignedSeat: () => void }) {
  const [name, setName] = useState(group.name);
  useEffect(() => setName(group.name), [group.id, group.name]);
  return <section className="group-name-editor" aria-label="Inspector do grupo"><header><span>GRUPO SELECIONADO</span><strong>{group.name}</strong></header><dl><div><dt>Tipo</dt><dd>{group.groupType === "workstation" ? "Estação" : group.groupType === "meeting" ? "Reunião" : group.groupType === "break_area" ? "Pausa" : group.groupType === "partition" ? "Setor" : "Personalizado"}</dd></div><div><dt>Móveis</dt><dd>{memberCount}</dd></div></dl><div className="group-members" aria-label="Membros do grupo"><span>MEMBROS</span>{members.map((member) => <div key={member.id}><strong>{member.name}</strong><small>{member.category}</small></div>)}</div>{group.groupType === "workstation" && <><button className="group-agent" disabled={!assignedAgent} aria-label={assignedAgent ? `Selecionar ${assignedAgent.name} · ${assignedAgent.role}` : "Estação sem agente associado"} onClick={onSelectAssignedAgent}><span>AGENTE ASSOCIADO</span><strong>{assignedAgent ? `${assignedAgent.name} · ${assignedAgent.role}` : "Sem agente associado"}</strong></button><button className="group-seat" disabled={!assignedSeatName} aria-label={assignedSeatName ? `Focar cadeira ${assignedSeatName}` : "Estação sem cadeira associada"} onClick={onFocusAssignedSeat}><span>CADEIRA ASSOCIADA</span><strong>{assignedSeatName ?? "Sem cadeira associada"}</strong></button></>}<label>{group.groupType === "workstation" ? "Nome da estação" : "Nome do grupo"}<input aria-label={group.groupType === "workstation" ? "Nome da estação" : "Nome do grupo"} value={name} maxLength={128} onChange={(event) => setName(event.target.value)} /></label><div className="group-name-actions"><button className="button" onClick={onFocus}>Focar grupo</button><button className="button" disabled={selectedMemberCount === memberCount} onClick={onSelectAll}>Selecionar todos</button><button className="primary" disabled={!name.trim() || name.trim() === group.name} onClick={() => onRename(name)}>Salvar nome</button></div></section>;
}

function FurnitureSelectionInspector({ name, category, position, orientation, groupName, seatCount, isSurfaceObject }: { name: string; category: string; position: { x: number; y: number }; orientation: string; groupName?: string; seatCount: number; isSurfaceObject: boolean }) {
  return <section className="furniture-selection-inspector" aria-label="Inspector do móvel"><header><span>MÓVEL SELECIONADO</span><strong>{name}</strong></header><dl><div><dt>Categoria</dt><dd>{category}</dd></div><div><dt>Posição</dt><dd>{position.x}, {position.y}</dd></div><div><dt>Orientação</dt><dd>{orientation}</dd></div><div><dt>Grupo</dt><dd>{groupName ?? "Sem grupo"}</dd></div></dl>{seatCount > 0 && <p>{seatCount === 1 ? "Possui 1 assento" : `Possui ${seatCount} assentos`}</p>}{isSurfaceObject && <p>Objeto sobre superfície</p>}</section>;
}
...
export function App() {
  ...
  const editableFurnitureGroup = selectedFurnitureGroup?.groupType === "lounge" ? undefined : selectedFurnitureGroup;
  const assignedWorkstationAgent = editableFurnitureGroup?.groupType === "workstation" ? agents.find((agent) => agent.id === assignedAgentIdForFurnitureGroup(editableFurnitureGroup, agentSeatAssignments)) : undefined;
  const assignedWorkstationSeat = assignedWorkstationAgent ? furniture.find((item) => item.id === agentSeatAssignments[assignedWorkstationAgent.id]) : undefined;
  const selectedGroupMembers = editableFurnitureGroup?.instanceIds.flatMap((id) => {
    const item = furniture.find((value) => value.id === id), asset = item && furnitureAsset(item.assetId);
    return item && asset ? [{ id, name: asset.name, category: asset.category }] : [];
  }) ?? [];
  ...
  const createMeetingPreset = useSceneStore((state) => state.createMeetingPreset);
  const createBreakAreaPreset = useSceneStore((state) => state.createBreakAreaPreset);
  const createPartitionPreset = useSceneStore((state) => state.createPartitionPreset);
  ...
  const [activePanel, setActivePanel] = useState<PrimaryPanel>(null);
  const [commandPaletteOpen, setCommandPaletteOpen] = useState(false);
  const [theme, setTheme] = useState<Theme>(() => (localStorage.getItem("agents-room-theme") as Theme | null) ?? "dark");
  ...
  const openPanel = (panel: Exclude<PrimaryPanel, null>) => setActivePanel((current) => toggledPanel(current, panel));
  ...
  useEffect(() => {
    const onSelect = (event: Event) => { const id = (event as CustomEvent<string>).detail; select(id); if (id) selectFurniture(undefined); };
    ...
    const onFurnitureSelect = (event: Event) => { const { id, additive } = (event as CustomEvent<{ id?: string; additive?: boolean }>).detail; if (id) select(undefined); selectFurniture(id, additive); };
  ...
  useEffect(() => { localStorage.setItem("agents-room-theme", theme); }, [theme]);
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      const editingText = ignoresWorkspaceShortcut(event.target);
      ...
      if (event.key === "Escape") {
        ...
        if (placingFurnitureAssetId) return;
        if (selectedFurnitureId) { selectFurniture(undefined); return; }
      ...
      const panel = panelForShortcut(event.key);
      if (panel) { event.preventDefault(); openPanel(panel); }
    };
    window.addEventListener("keydown", onKeyDown, true);
    return () => window.removeEventListener("keydown", onKeyDown, true);
  }, [activePanel, commandPaletteOpen, placingFurnitureAssetId, selectedFurnitureId, selectedId, select, selectFurniture, toggleEdit]);
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      const control = event.target instanceof HTMLElement ? event.target.closest("input, textarea, select, button, [contenteditable=true]") : undefined;
      if (!editMode || (control && !control.matches("button"))) return;
      if (event.key === "Enter") { if (control || !assignedWorkstationAgent) return; event.preventDefault(); select(assignedWorkstationAgent.id); return; }
  ...
  }, [assignedWorkstationAgent, canRotateSelectedFurniture, editMode, placingFurnitureAssetId, redoFurniture, rotateFurniturePlacement, select, selectedFurnitureId, undoFurniture]);
  useEffect(() => {
  ...
  const createDualMonitorWorkstation = () => {
    if (!selected) return;
    if (!createWorkstationPreset(selected.id, selected.basePosition, 2)) { reportError("Não há espaço livre para montar esta estação."); return; }
    setEvents((items) => [`Estação com dois monitores criada para ${selected.name}.`, ...items].slice(0, 10));
  };
  const createLounge = () => {
  ...
  const createMeeting = () => {
    if (!createMeetingPreset()) { reportError("Não há espaço livre para montar a área de reunião."); return; }
    setEvents((items) => ["Área de reunião com dois assentos criada.", ...items].slice(0, 10));
  };
  const createBreakArea = () => {
    if (!createBreakAreaPreset()) { reportError("Não há espaço livre para montar a área de pausa."); return; }
    setEvents((items) => ["Área de pausa com café e água criada.", ...items].slice(0, 10));
  };
  const createPartition = () => {
    if (!createPartitionPreset()) { reportError("Não há espaço livre para montar a divisória."); return; }
    setEvents((items) => ["Divisória setorial criada.", ...items].slice(0, 10));
  };
  ...
  const renameGroup = (name: string) => {
    if (!renameSelectedFurnitureGroup(name)) { reportError("Informe um nome de grupo ou estação válido."); return; }
  ...
  const selectAssignedWorkstationAgent = () => {
    if (assignedWorkstationAgent) { selectFurniture(undefined); select(assignedWorkstationAgent.id); }
  };
  ...
  const commandActions: Array<[string, string, () => void]> = [["Agentes", "A", () => openPanel("agents")], ["Tarefas", "T", () => openPanel("tasks")], ["Skills e plugins", "S", () => openPanel("capabilities")], ["Galeria de móveis", "G", () => openPanel("furniture")], [editMode ? "Concluir edição" : "Modo edição", "E", toggleEdit]];

  return <DndContext collisionDetection={pointerWithin} onDragEnd={onDragEnd}>
    <main className="workspace-shell" data-theme={theme}>
      <header className="compact-topbar">
        <div className="brand"><span className="brand-mark">AR</span><div><strong>Agents Room</strong><small>{workspaceQuery.data?.gitBranch ?? "workspace local"}</small></div></div>
        <div className="topbar-project"><span>agents-room</span><span className={workspaceQuery.data ? "codex-online" : "codex-offline"}>● {workspaceQuery.data ? "Codex local" : "Offline"}</span></div>
        <div className="topbar-actions"><button className="icon-button" aria-label="Busca global" title="Busca global (⌘K)" onClick={() => setCommandPaletteOpen(true)}><Search size={18} /></button><button className="icon-button" aria-label="Alternar modo edição" title="Modo edição (E)" onClick={toggleEdit}><Edit3 size={18} /></button><button className="icon-button notification-button" aria-label="Aprovações pendentes" title="Aprovações pendentes" onClick={() => openPanel("tasks")}><ClipboardList size={18} />{Boolean(approvalsQuery.data?.length) && <b>{approvalsQuery.data?.length}</b>}</button><AddAgentDialog onCreate={createAgent} /></div>
      </header>
      <nav className="side-rail" aria-label="Navegação do workspace">
        <button className={activePanel === "overview" ? "rail-button active" : "rail-button"} aria-label="Visão geral" title="Visão geral" onClick={() => openPanel("overview")}><Home size={20} /></button>
        <button className={activePanel === "agents" ? "rail-button active" : "rail-button"} aria-label="Agentes" title="Agentes (A)" onClick={() => openPanel("agents")}><Bot size={20} /></button>
        <button className={activePanel === "tasks" ? "rail-button active" : "rail-button"} aria-label="Tarefas" title="Tarefas (T)" onClick={() => openPanel("tasks")}><ClipboardList size={20} /></button>
        <button className={activePanel === "capabilities" ? "rail-button active" : "rail-button"} aria-label="Skills e plugins" title="Skills e plugins (S)" onClick={() => openPanel("capabilities")}><Layers3 size={20} /></button>
        <button className={activePanel === "furniture" ? "rail-button active" : "rail-button"} aria-label="Galeria de móveis" title="Galeria de móveis (G)" onClick={() => openPanel("furniture")}><Box size={20} /></button>
        <button className={editMode ? "rail-button active" : "rail-button"} aria-label="Modo edição" title="Modo edição (E)" onClick={toggleEdit}><Grid3X3 size={20} /></button>
        <button className={activePanel === "activity" ? "rail-button active" : "rail-button"} aria-label="Atividade" title="Atividade" onClick={() => openPanel("activity")}><Activity size={20} /><i>{events.length}</i></button>
        <span className="rail-spacer" />
        <button className="rail-button" aria-label="Ajuda" title="Ajuda e atalhos" onClick={() => setCommandPaletteOpen(true)}><CircleHelp size={20} /></button>
        <button className={activePanel === "settings" ? "rail-button active" : "rail-button"} aria-label="Configurações" title="Configurações" onClick={() => openPanel("settings")}><Settings size={20} /></button>
      </nav>
      <section className="scene-stage">
        <SceneDrop><OfficeCanvas /></SceneDrop>
        <div className="scene-status"><span className={editMode ? "mode edit" : "mode"}>{placingFurnitureAssetId ? `Posicionando ${furnitureAsset(placingFurnitureAssetId)?.name}` : editMode ? "Modo edição" : "Modo operação"}</span>{selected && <button className="selection-pill" onClick={() => select(undefined)}><span className={`status-dot ${selected.status}`} />{selected.name}<X size={14} /></button>}</div>
        {lastError && <p className="scene-error" role="alert">{lastError}<button aria-label="Fechar erro" onClick={() => setLastError(undefined)}><X size={15} /></button></p>}
        {activePanel && <aside className="workspace-drawer" aria-label={PANEL_LABEL[activePanel]}><header><div><p>WORKSPACE</p><h2>{PANEL_LABEL[activePanel]}</h2></div><button className="icon-button" aria-label="Fechar painel" onClick={() => setActivePanel(null)}><PanelLeftClose size={18} /></button></header><div className="drawer-body">
          {activePanel === "overview" && <><div className="overview-cards"><article><Bot size={17} /><strong>{agents.length}/8</strong><span>Agentes ativos</span></article><article><ClipboardList size={17} /><strong>{tasksQuery.data?.filter((item) => ["queued", "starting", "running"].includes(item.state)).length ?? 0}</strong><span>Tarefas em curso</span></article><article><Check size={17} /><strong>{approvalsQuery.data?.length ?? 0}</strong><span>Aprovações</span></article></div><p className="section-label">ATIVIDADE RECENTE</p>{events.slice(0, 4).map((event, index) => <p className="drawer-event" key={`${event}-${index}`}>{event}</p>)}</>}
          {activePanel === "agents" && <><div className="drawer-search"><Search size={16} /><input aria-label="Buscar agentes" placeholder="Buscar por nome" onChange={(event) => setCatalogSearch(event.target.value)} value={catalogSearch} /></div><AddAgentDialog onCreate={createAgent} /><div className="agent-list">{agents.filter((agent) => agent.name.toLocaleLowerCase().includes(catalogSearch.trim().toLocaleLowerCase())).map((agent) => <button key={agent.id} className={agent.id === selectedId ? "agent-list-item selected" : "agent-list-item"} onClick={() => { selectFurniture(undefined); select(agent.id); setActivePanel(null); }}><span className="avatar" style={{ background: `#${agent.color.toString(16)}` }}>{agent.name[0]}</span><span><strong>{agent.name}</strong><small>{agent.role} · {agent.status}</small></span><span className={`status-dot ${agent.status}`} /></button>)}</div></>}
          {activePanel === "tasks" && <><label>Permissão da próxima tarefa<select value={taskAccess} onChange={(event) => setTaskAccess(event.target.value)}><option value="read_only">Somente leitura</option><option value="workspace_write">Escrita no workspace</option></select></label><p className="section-label">APROVAÇÕES</p>{approvalsQuery.data?.length ? approvalsQuery.data.map((approval) => <article key={approval.id} className="approval-card"><p>{approval.summary}</p><div><button className="primary" onClick={() => decideApproval(approval.id, true)}>Aprovar</button><button className="button" onClick={() => decideApproval(approval.id, false)}>Rejeitar</button></div></article>) : <div className="empty-plugin">Nenhuma aprovação pendente.</div>}<p className="section-label">TAREFAS DE {selected?.name ?? "AGENTE"}</p>{selected ? tasksQuery.data?.length ? tasksQuery.data.map((item) => <article key={item.id} className="task-card"><strong>{item.state}</strong><p>{item.prompt}</p>{["queued", "running", "waiting_approval"].includes(item.state) && <button className="button" onClick={() => cancelTask(item.id)}>Cancelar</button>}</article>) : <div className="empty-plugin">Nenhuma tarefa recente.</div> : <div className="empty-plugin">Selecione um agente para ver as tarefas.</div>}</>}
          {activePanel === "capabilities" && <><div className="drawer-search"><Search size={16} /><input aria-label="Buscar skills" placeholder="Buscar skill" value={catalogSearch} onChange={(event) => setCatalogSearch(event.target.value)} /></div><select className="catalog-filter" aria-label="Filtrar skills por categoria" value={catalogCategory} onChange={(event) => setCatalogCategory(event.target.value)}><option value="all">Todas as categorias</option>{[...new Set(catalogSkills.map((skill) => skill.category))].map((category) => <option key={category} value={category}>{category}</option>)}</select><p className="section-label">SKILLS</p>{filteredSkills.length ? filteredSkills.map((skill) => <DraggableSkill key={skill.id} skill={skill} disabled={!selected} onAssign={assignSkillToSelected} />) : <div className="empty-plugin">Nenhuma skill encontrada.</div>}<p className="section-label">PLUGINS</p>{pluginsQuery.data?.length ? pluginsQuery.data.map((plugin) => <button key={plugin.id} className="skill-card" onClick={() => assignPlugin(plugin.id)} disabled={!selected}><span>+ {plugin.name}</span><small>{plugin.description}</small></button>) : <div className="empty-plugin">Nenhum plugin disponível.</div>}</>}
          {activePanel === "furniture" && <><div className="drawer-search"><Search size={16} /><input aria-label="Buscar móveis" placeholder="Buscar móvel" value={furnitureSearch} onChange={(event) => setFurnitureSearch(event.target.value)} /></div><select className="catalog-filter" aria-label="Filtrar móveis por categoria" value={furnitureCategory} onChange={(event) => setFurnitureCategory(event.target.value as FurnitureCategory | "all")}><option value="all">Todas as categorias</option>{[...new Set(FURNITURE_ASSETS.map((asset) => asset.category))].map((category) => <option key={category} value={category}>{category}</option>)}</select><div className="preset-actions"><button className="primary" disabled={!editMode || !selected} onClick={createWorkstation}>Estação</button><button className="button" disabled={!editMode || !selected} onClick={createDualMonitorWorkstation}>Estação dupla</button><button className="button" disabled={!editMode} onClick={createLounge}>Lounge</button><button className="button" disabled={!editMode} onClick={createMeeting}>Reunião</button><button className="button" disabled={!editMode} onClick={createBreakArea}>Pausa</button><button className="button" disabled={!editMode} onClick={createPartition}>Divisória</button></div><p className="section-label">MÓVEIS</p><div className="furniture-gallery">{filteredFurniture.map((asset) => <button key={asset.id} className="furniture-tile" disabled={!editMode || Boolean(asset.surface && !selectedFurnitureId)} onClick={() => addCatalogFurniture(asset)}><strong>{asset.name}</strong><small>{asset.surface ? "superfície" : asset.category}{furnitureSeats(asset).length ? " · assento" : ""}</small></button>)}</div></>}
          {activePanel === "activity" && <><p className="section-label">STREAM LOCAL</p><ol className="activity-list" aria-live="polite">{events.map((event, index) => <li key={`${event}-${index}`}><time>agora</time>{event}</li>)}</ol></>}
          {activePanel === "settings" && <><p className="section-label">TEMA</p><div className="theme-options">{(["dark", "light", "system"] as Theme[]).map((value) => <button key={value} className={theme === value ? "theme-option active" : "theme-option"} onClick={() => setTheme(value)}>{value === "dark" ? <Moon size={16} /> : value === "light" ? <Sun size={16} /> : <Eye size={16} />}{value === "dark" ? "Escuro" : value === "light" ? "Claro" : "Sistema"}</button>)}</div><p className="section-label">SALA</p><button className="button full" onClick={() => setPendingLayoutAction("restore")}>Restaurar layout padrão</button>{import.meta.env.DEV && <a className="button asset-editor-link" href="/dev/asset-editor">Abrir editor de assets</a>}</>}
        </div></aside>}
        {selected && <aside className="context-inspector agent-context" aria-label={`Inspector de ${selected.name}`}><header><div className="agent-title"><span className="avatar" style={{ background: `#${selected.color.toString(16)}` }}>{selected.name[0]}</span><div><h2>{selected.name}</h2><p>{selected.role}</p></div></div><button className="icon-button" aria-label="Fechar inspector" onClick={() => select(undefined)}><X size={18} /></button></header><p className="description">{selected.description}</p><dl className="details"><div><dt>Estado</dt><dd>{selected.status}</dd></div><div><dt>Sessão</dt><dd>{selected.sessionId ? `Codex ${selected.sessionId.slice(0, 8)}` : "Aguardando tarefa"}</dd></div></dl><details open><summary>Tarefa atual</summary><textarea value={task} onChange={(event) => setTaskDraft(event.target.value)} placeholder="Descreva uma tarefa…" /><button className="primary full" onClick={sendTask}>Enviar ao Codex</button>{selected.task && <p className="task-current">Em execução: {selected.task}</p>}</details><details><summary>Estação e capacidades</summary><label>Cadeira principal<select aria-label="Cadeira principal" value={agentSeatAssignments[selected.id] ?? ""} onChange={(event) => assignAgentSeat(selected.id, event.target.value || undefined)}><option value="">Sem cadeira associada</option>{seatFurniture.filter((item) => !Object.entries(agentSeatAssignments).some(([agentId, seatId]) => agentId !== selected.id && seatId === item.id)).map((item) => <option key={item.id} value={item.id}>{furnitureAsset(item.assetId)?.name}</option>)}</select></label><SkillDropZone>{selected.skillStates.length ? selected.skillStates.map(({ id, enabled }) => <span key={id} className={enabled ? "skill-chip" : "skill-chip disabled"}>{catalogSkills.find((skill) => skill.id === id)?.name ?? id}<button onClick={() => updateSkill(id, !enabled)}>{enabled ? "Pausar" : "Ativar"}</button><button onClick={() => removeSkill(id)} aria-label={`Remover ${id}`}>×</button></span>) : <span className="empty">Abra Skills para atribuir</span>}</SkillDropZone></details><details><summary>Interagir</summary><select aria-label="Tipo da interação" value={interactionKind} onChange={(event) => setInteractionKind(event.target.value as "context_share" | "delegation")}><option value="context_share">Compartilhar contexto</option><option value="delegation">Delegar subtarefa</option></select><select aria-label="Agente de destino" value={interactionTarget} onChange={(event) => setInteractionTarget(event.target.value)}><option value="">Escolha um agente</option>{agents.filter((agent) => agent.id !== selected.id).map((agent) => <option key={agent.id} value={agent.id}>{agent.name}</option>)}</select><input value={interactionSummary} onChange={(event) => setInteractionSummary(event.target.value)} aria-label="Resumo da interação" /><button className="button full" onClick={requestInteraction}>{interactionKind === "delegation" ? "Delegar" : "Solicitar interação"}</button></details><button className="danger-link" onClick={deleteSelected}>Remover agente</button></aside>}
        {editMode && selectedFurniture && furnitureAsset(selectedFurniture.assetId) && <aside className="context-inspector furniture-context"><FurnitureSelectionInspector name={furnitureAsset(selectedFurniture.assetId)!.name} category={furnitureAsset(selectedFurniture.assetId)!.category} position={selectedFurniture.position} orientation={selectedFurniture.orientation} groupName={selectedFurnitureGroup?.name} seatCount={furnitureSeats(furnitureAsset(selectedFurniture.assetId)!).length} isSurfaceObject={Boolean(selectedFurniture.parentId)} /><div className="inspector-actions"><button className="button" disabled={!canRotateSelectedFurniture} onClick={() => requestFurnitureRotation(selectedFurniture.id)}>Rotacionar</button><button className="button" onClick={() => requestFurnitureDuplicate(selectedFurniture.id)}>Duplicar</button><button className="danger-link" onClick={() => requestFurnitureDeletion(selectedFurniture.id)}>Excluir</button></div></aside>}
        {editableFurnitureGroup && <aside className="context-inspector group-context"><GroupNameEditor group={editableFurnitureGroup} memberCount={editableFurnitureGroup.instanceIds.length} members={selectedGroupMembers} selectedMemberCount={selectedFurnitureIds.filter((id) => editableFurnitureGroup.instanceIds.includes(id)).length} assignedAgent={assignedWorkstationAgent} assignedSeatName={assignedWorkstationSeat && furnitureAsset(assignedWorkstationSeat.assetId)?.name} onRename={renameGroup} onSelectAll={selectFurnitureGroup} onFocus={focusFurnitureGroup} onSelectAssignedAgent={selectAssignedWorkstationAgent} onFocusAssignedSeat={focusAssignedWorkstationSeat} /></aside>}
        <div className="scene-controls"><button className="icon-button" aria-label="Aproximar" title="Aproximar" onClick={() => window.dispatchEvent(new CustomEvent("scene:camera-control", { detail: "zoom-in" }))}><ZoomIn size={18} /></button><button className="icon-button" aria-label="Afastar" title="Afastar" onClick={() => window.dispatchEvent(new CustomEvent("scene:camera-control", { detail: "zoom-out" }))}><ZoomOut size={18} /></button><button className="icon-button" aria-label="Focar seleção" title="Focar seleção (F)" onClick={() => window.dispatchEvent(new CustomEvent("scene:camera-control", { detail: "focus" }))}><Eye size={18} /></button></div>
        {editMode && <div className="edit-toolbar"><button className="icon-button" aria-label="Desfazer" title="Desfazer" onClick={undoFurniture}>↶</button><button className="icon-button" aria-label="Refazer" title="Refazer" onClick={redoFurniture}>↷</button><button className="button" disabled={!selectedFurnitureId || !canRotateSelectedFurniture} onClick={() => selectedFurnitureId && requestFurnitureRotation(selectedFurnitureId)}>Rotacionar</button><button className="button" disabled={!selectedFurnitureId} onClick={() => selectedFurnitureId && requestFurnitureDuplicate(selectedFurnitureId)}>Duplicar</button><button className="button" onClick={toggleEdit}>Concluir</button></div>}
      </section>
      <Dialog.Root open={commandPaletteOpen} onOpenChange={setCommandPaletteOpen}><Dialog.Portal><Dialog.Overlay className="dialog-overlay" /><Dialog.Content className="command-palette"><Dialog.Title><Command size={18} />Ações rápidas</Dialog.Title><Dialog.Description>Atalhos principais do workspace.</Dialog.Description><div>{commandActions.map(([label, shortcut, action]) => <button key={label} onClick={() => { action(); setCommandPaletteOpen(false); }}><span>{label}</span><kbd>{shortcut}</kbd></button>)}</div></Dialog.Content></Dialog.Portal></Dialog.Root>
  ...
  /* Legacy full-time panel layout retained below only until the shell migration is reviewed.
  return <DndContext collisionDetection={pointerWithin} onDragEnd={onDragEnd}><main className="app-shell">
    ...
    <Dialog.Root open={Boolean(pendingFurnitureDeletion)} onOpenChange={(open) => { if (!open) setPendingFurnitureDeletion(undefined); }}><Dialog.Portal><Dialog.Overlay className="dialog-overlay" /><Dialog.Content className="dialog"><Dialog.Title>Remover móvel em uso?</Dialog.Title><Dialog.Description>{pendingFurnitureDeletion?.agentNames.join(", ")} {pendingFurnitureDeletion?.agentNames.length === 1 ? "perderá a estação ou assento associado." : "perderão a estação ou assento associado."}</Dialog.Description><div className="dialog-actions"><Dialog.Close className="button">Cancelar</Dialog.Close><button className="primary" onClick={() => { if (pendingFurnitureDeletion) window.dispatchEvent(new CustomEvent("furniture:delete-force", { detail: pendingFurnitureDeletion.id })); setPendingFurnitureDeletion(undefined); }}>Remover mesmo assim</button></div></Dialog.Content></Dialog.Portal></Dialog.Root>
    <Dialog.Root open={Boolean(pendingLayoutAction)} onOpenChange={(open) => { if (!open) setPendingLayoutAction(undefined); }}><Dialog.Portal><Dialog.Overlay className="dialog-overlay" /><Dialog.Content className="dialog"><Dialog.Title>{pendingLayoutAction === "clear" ? "Limpar escritório?" : "Restaurar layout padrão?"}</Dialog.Title><Dialog.Description>{pendingLayoutAction === "clear" ? `Isso removerá ${furniture.length} móveis e ${Object.keys(agentSeatAssignments).length} associações de cadeira.` : `Isso substituirá ${furniture.length} móveis e ${Object.keys(agentSeatAssignments).length} associações pelas estações dos ${agents.length} agentes atuais.`} Você poderá desfazer a alteração.</Dialog.Description><div className="dialog-actions"><Dialog.Close className="button">Cancelar</Dialog.Close><button className={pendingLayoutAction === "clear" ? "danger-button" : "primary"} onClick={applyLayoutAction}>{pendingLayoutAction === "clear" ? "Limpar sala" : "Restaurar padrão"}</button></div></Dialog.Content></Dialog.Portal></Dialog.Root>
  </main>{editMode && selectedFurniture && furnitureAsset(selectedFurniture.assetId) && <FurnitureSelectionInspector name={furnitureAsset(selectedFurniture.assetId)!.name} category={furnitureAsset(selectedFurniture.assetId)!.category} position={selectedFurniture.position} orientation={selectedFurniture.orientation} groupName={selectedFurnitureGroup?.name} seatCount={furnitureSeats(furnitureAsset(selectedFurniture.assetId)!).length} isSurfaceObject={Boolean(selectedFurniture.parentId)} />}{editableFurnitureGroup && <GroupNameEditor group={editableFurnitureGroup} memberCount={editableFurnitureGroup.instanceIds.length} members={selectedGroupMembers} selectedMemberCount={selectedFurnitureIds.filter((id) => editableFurnitureGroup.instanceIds.includes(id)).length} assignedAgent={assignedWorkstationAgent} assignedSeatName={assignedWorkstationSeat && furnitureAsset(assignedWorkstationSeat.assetId)?.name} onRename={renameGroup} onSelectAll={selectFurnitureGroup} onFocus={focusFurnitureGroup} onSelectAssignedAgent={selectAssignedWorkstationAgent} onFocusAssignedSeat={focusAssignedWorkstationSeat} />}</DndContext>;
  */
}
