...
export function App() {
  ...
  useEffect(() => { if (officeLayoutQuery.data) replaceOfficeLayout(officeLayoutQuery.data.furnitureInstances, officeLayoutQuery.data.agentSeatAssignments ?? {}); }, [officeLayoutQuery.data, replaceOfficeLayout]);
  useEffect(() => {
    if (!workspaceQuery.data || !officeLayoutQuery.isSuccess) return;
    const timer = window.setTimeout(() => { void api.saveOfficeLayout(workspaceQuery.data!.id, furniture, agentSeatAssignments).catch(() => reportError("Não foi possível salvar o layout da sala.")); }, 350);
    return () => window.clearTimeout(timer);
  }, [agentSeatAssignments, furniture, officeLayoutQuery.isSuccess, workspaceQuery.data]);
  useEffect(() => {
