...
function GroupNameEditor({ group, memberCount, selectedMemberCount, onRename, onSelectAll, onFocus }: { group: { id: string; name: string; instanceIds: string[] }; memberCount: number; selectedMemberCount: number; onRename: (name: string) => void; onSelectAll: () => void; onFocus: () => void }) {
...
export function App() {
  ...
  </main>{selectedFurnitureGroup?.groupType === "custom" && <GroupNameEditor group={selectedFurnitureGroup} memberCount={selectedFurnitureGroup.instanceIds.length} selectedMemberCount={selectedFurnitureIds.filter((id) => selectedFurnitureGroup.instanceIds.includes(id)).length} onRename={renameGroup} onSelectAll={selectFurnitureGroup} onFocus={focusFurnitureGroup} />}</DndContext>;
}
