...
describe("furniture catalog", () => {
  ...
  it("uses only supplied images for supported orientations", () => {
    ...
    const meetingDesk = FURNITURE_ASSETS.find((asset) => asset.id === "desk.meeting.l.01")!;
    expect(furnitureOrientations(meetingDesk)).toEqual(["north_east", "south_east"]);
    expect(furnitureImage(meetingDesk, "south_east")).toContain("desk-meeting-l-se-01.png");
