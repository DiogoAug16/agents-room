...
import type { AgentSeatAssignments, FurnitureInstance } from "../scene/furniture/catalog";
...
export const api = {
  ...
  officeLayout: (workspaceId: string) => request<{ schemaVersion: number; furnitureInstances: FurnitureInstance[]; agentSeatAssignments: AgentSeatAssignments }>(`/workspaces/${workspaceId}/office-layout`),
  saveOfficeLayout: (workspaceId: string, furniture: FurnitureInstance[], agentSeatAssignments: AgentSeatAssignments) => request(`/workspaces/${workspaceId}/office-layout`, { method: "PUT", body: JSON.stringify({ schema_version: 2, furniture_instances: furniture, agent_seat_assignments: agentSeatAssignments }) }),
