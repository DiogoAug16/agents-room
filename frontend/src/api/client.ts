...
type ApiAgent = { id: string; name: string; role: string; description: string; visualStatus: VisualStatus; position: { x: number; y: number }; basePosition: { x: number; y: number }; direction: Agent["direction"]; skills: { id: string; enabled: boolean }[] };
...
export const api = {
  ...
  agents: async (workspaceId: string): Promise<Agent[]> => (await request<ApiAgent[]>(`/workspaces/${workspaceId}/agents`)).map((agent, index) => ({ ...agent, status: agent.visualStatus, color: colors[index % colors.length], skills: agent.skills.map((skill) => skill.id), skillStates: agent.skills })),
